        ...
        try:
            ...
            for current_model in models_to_try:
                for attempt in range(max_retries):
                    # Modo PDF: sempre sem grounding (determinístico e econômico)
                    # Modo WebSearch: com grounding nas 2 primeiras tentativas, sem grounding na 3ª
                    if sumula_raw_text:
                        use_cfg = config_no_grounding
                        mode_label = f"Modo PDF Direto (Tentativa {attempt+1}) - Sem Web Search"
                    else:
