...
class MonitorApp(MonitorCoreMixin, ctk.CTk):
    def __init__(self) -> None:
        ...
        self.title("Expert Assistant & Strategic Monitoring - Mediadna")
    ...
    def _build_tab_config(self) -> None:
        ...
        ctk.CTkSwitch(
            email_wrap,
            text="Buscar e auditar jogos da CBF automaticamente (Auditoria Automática)",
        ...
        ctk.CTkLabel(email_input_row, text="Destinatários (separar por ;):").pack(side="left", padx=(0, 5))
        self.email_entry = ctk.CTkEntry(email_input_row, textvariable=self.email_recipients_var, placeholder_text="ex: admin@email.com; user@email.com", width=400)
        ...
        def worker():
            try:
                ...
                for idx, ev in enumerate(events):
                    ...
                    # 3. Executar Análise
                    try:
                        self.after(0, lambda i=idx, t=title: update_popup(i, t, f"Consultando {model_id.title()} (Grounding)..."))
                        ...
                        for key, label, def_min in chrono_map:
                            ...
                            if val and val != "N/A" and label not in seen_labels:
                                ...
                                to_render.append({
                                    ...
                                    "label": label,
                                    "clock": val,
                                    "summary": f"{label} detectado pela Auditoria I.A.",
                ...
                for key, label, def_min in chrono_map:
                    ...
                    if val and val != "N/A":
                        ...
                        to_render.append({
                            ...
                            "label": label,
                            "clock": val,
                            "summary": f"{label} detectado via Auditoria I.A.",
    ...
    def _show_report_completed_dialog(self, pdf_path: str) -> None:
        ...
        # Campo de E-mails Destinatários pré-preenchido
        email_row = ctk.CTkFrame(frame, fg_color="transparent")
        email_row.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(email_row, text="E-mails:", font=ctk.CTkFont(size=11)).pack(side="left", padx=(0, 5))
        
        email_var = ctk.StringVar(value=self.email_recipients_var.get())
        email_entry = ctk.CTkEntry(email_row, textvariable=email_var, width=320, placeholder_text="destinatario@email.com")
        email_entry.pack(side="left", fill="x", expand=True)

        def _send_email_action():
            recipients = [em.strip() for em in email_var.get().split(";") if em.strip()]
            if not recipients:
                messagebox.showerror("Erro", "Insira pelo menos um e-mail válido para envio.")
                return
            
            # Atualiza destinatários na config global
            self.email_recipients_var.set(email_var.get())
            self._save_general_settings()

            self._log(f"[EMAIL] Disparando envio manual para: {email_var.get()}")
            threading.Thread(target=self._send_report_via_email, args=(pdf_path,), daemon=True).start()
...
📄 SÚMULA ELETRÔNICA CBF: 100% Validada"""
    ...
    def _open_schedule_games_modal(self) -> None:
        ...
        ctk.CTkLabel(email_frame, text="📧 E-mails de Destino Cadastrados:", font=ctk.CTkFont(size=11, weight="bold"), text_color="#00CED1").pack(anchor="w", padx=10, pady=(6, 2))
        
        saved_emails = _load_saved_emails()
        email_vars = {}
        chk_row = ctk.CTkFrame(email_frame, fg_color="transparent")
        chk_row.pack(fill="x", padx=10, pady=(0, 6))
        
        for em in saved_emails:
            f_em = ctk.CTkFrame(chk_row, fg_color="transparent")
            f_em.pack(side="left", padx=4)
            var = ctk.BooleanVar(value=True)
            email_vars[em] = var
            ctk.CTkCheckBox(f_em, text=em, variable=var, font=ctk.CTkFont(size=11)).pack(side="left", padx=(2, 2))
            
            def _make_del_cmd(_target=em, _frame=f_em):
                def _del():
                    _remove_saved_email(_target)
                    if _target in email_vars:
                        del email_vars[_target]
                    _frame.destroy()
                    self._log(f"🗑️ E-mail '{_target}' removido dos destinatários salvos.")
                return _del
            ...
            ctk.CTkButton(
                ...
                fg_color="#222222",
                hover_color="#8B0000",
                command=_make_del_cmd(em, f_em)
            ).pack(side="left", padx=(0, 4))

        # Adicionar novo e-mail
        new_email_frame = ctk.CTkFrame(email_frame, fg_color="transparent")
        new_email_frame.pack(fill="x", padx=10, pady=(0, 8))
        ctk.CTkLabel(new_email_frame, text="Outros e-mails:").pack(side="left", padx=(0, 5))
        other_email_entry = ctk.CTkEntry(new_email_frame, width=320, placeholder_text="Ex: novo.destinatario@mediadna.com.br")
        other_email_entry.pack(side="left", padx=5)
        ...
        def _save_schedule():
            ...
            extra = other_email_entry.get().strip()
            if extra:
                target_emails.append(extra)
                _save_email(extra)
    ...
    def _update_schedule_panel_ui(self) -> None:
        """Renderiza a lista de jogos agendados no painel lateral com estatutos em tempo real e relógios regressivos."""
        ...
        for g in games:
            ...
            ctk.CTkLabel(card, text=f"{t1} x {t2}", font=ctk.CTkFont(size=11, weight="bold"), text_color="#FFFFFF").pack(anchor="w", padx=8, pady=(4, 1))
            ...
            if countdown_text:
                ctk.CTkLabel(card, text=countdown_text, font=ctk.CTkFont(size=9), text_color="#AAAAAA").pack(anchor="w", padx=8, pady=(0, 4))
            ...
            if g.get("status", "pending") == "pending":
                ...
                try:
                    ...
                    if now >= e_dt:
                        g["status"] = "running"
                        updated = True
                        self._log(f"⏰ [AGENDAMENTO] Disparando auditoria automática de {g.get('team1')} x {g.get('team2')}...")
                        
                        # Disparar em thread separada para não travar a GUI
                        def run_scheduled_audit_thread(_game=g):
                            try:
                                # Carregar chaves
                                cfg_path = os.path.join(PROJECT_ROOT, "config", "google_ai.json")
                                with open(cfg_path, "r", encoding="utf-8") as f:
                                ...
                                if "error" not in result:
                                    ...
                                    # Injetar marcos no detector e grid
                                    to_render = []
                                    seen_labels = set()
                                    chrono_map = [
                                        ("pre_game_start", "INÍCIO TRANSMISSÃO", -10),
                                    ...
                                    # Enviar por e-mail se configurado no agendamento
                                    ...
                                    self._log(f"⏰ [AGENDAMENTO ERRO] IA retornou erro para {_game.get('team1')} x {_game.get('team2')}: {result.get('error')}")
                                    _game["status"] = "failed"
                                    
                            except Exception as ex_audit:
                                self._log(f"⏰ [AGENDAMENTO ERRO] Falha crítica na auditoria de {_game.get('team1')} x {_game.get('team2')}: {ex_audit}")
                                _game["status"] = "failed"
                            finally:
