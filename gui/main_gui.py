...
def _load_saved_emails() -> list:
    path = os.path.join(PROJECT_ROOT, "config", "saved_emails.json")
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            emails = json.load(f)
            # Garantir remoção do e-mail antigo do arthur caso persista no arquivo
            if "arthur@mediadna.com.br" in emails:
                emails.remove("arthur@mediadna.com.br")
                with open(path, "w", encoding="utf-8") as fw:
                    json.dump(emails, fw, indent=2)
            return emails
    except:
        return default_emails
...
def _load_scheduled_games() -> list:
    path = os.path.join(PROJECT_ROOT, "config", "scheduled_games.json")
    if not os.path.exists(path):
        return []
    try:
        with open(path, "r", encoding="utf-8") as f:
...
def _save_scheduled_games(games_list: list) -> None:
    ...
    try:
        ...
        print(f"[SCHEDULE WARN] Erro ao salvar agendamentos: {e}")

def _load_recent_searches() -> list:
    path = os.path.join(PROJECT_ROOT, "config", "recent_searches.json")
    default_searches = [
    ...
    if not os.path.exists(path):
        try:
            os.makedirs(os.path.dirname(path), exist_ok=True)
            with open(path, "w", encoding="utf-8") as f:
                json.dump(default_searches, f, indent=2, ensure_ascii=False)
        except: pass
...
def _save_recent_search(team1: str, team2: str, comp: str, date: str, platform: str) -> list:
    ...
    try:
        ...
        print(f"[PRESETS WARN] Erro ao salvar histórico de pesquisas: {e}")
    return searches
...
class MonitorApp(MonitorCoreMixin, ctk.CTk):
    ...
    def _create_history_card(self, entry: Dict[str, Any]) -> None:
        ...
        card = ctk.CTkFrame(self.history_grid, corner_radius=6, fg_color="#181818", border_width=1, border_color="#2c2c2c", height=32)
        ...
        ctk.CTkLabel(card, text=time_lbl, font=ctk.CTkFont(family="Consolas", size=11, weight="bold"), text_color="#ffffff").pack(side="left", padx=(8, 4), pady=4)
        ctk.CTkLabel(card, text=label or entry.get("type", "").upper(), font=ctk.CTkFont(size=11, weight="bold"), text_color=badge_color).pack(side="left", padx=4, pady=4)
        ...
        if summary:
            ctk.CTkLabel(card, text=f"• {summary[:85]}", font=ctk.CTkFont(size=11), text_color="#CCCCCC", anchor="w").pack(side="left", fill="x", expand=True, padx=6, pady=4)
        ...
        try:
            ...
            cbadge.pack(side="right", padx=6, pady=4)
        except: pass
    ...
    def _build_ui(self) -> None:
        ...
        self.btn_ai_alert = ctk.CTkButton(
            mode_bar,
            text="✨ Programa Inteligente",
            font=ctk.CTkFont(size=11, weight="bold"),
            width=160,
            fg_color="#00CED1",
            text_color="black",
            hover_color="#008B8B",
            command=lambda: messagebox.showinfo("Info", "Você está em um programa inteligente de IA")
        )
        self.btn_ai_alert.pack(side="left", padx=10, pady=10)
        ...
        # 1. Painel manual do modo Expert (SEMPRE FIXO NO TOPO)
        self.expert_manual_frame = ctk.CTkFrame(self, corner_radius=14, border_width=1, border_color="#333333")
        self.expert_manual_frame.pack(fill="x", padx=12, pady=(6, 4))
        ...
        ctk.CTkLabel(row_inputs, text="Time Casa:").pack(side="left", padx=(0, 4))
        ...
        ctk.CTkLabel(row_inputs, text="Time Fora:").pack(side="left", padx=(0, 4))
        ...
        ctk.CTkLabel(row_inputs, text="Competição:").pack(side="left", padx=(0, 4))
        ...
        ctk.CTkLabel(row_inputs, text="Data:").pack(side="left", padx=(0, 4))
        if DateEntry:
        ...
        ctk.CTkLabel(row_inputs, text="Plataforma:").pack(side="left", padx=(0, 4))
        ...
        ctk.CTkLabel(row_inputs, text="Horário (VOD):").pack(side="left", padx=(0, 4))
        ctk.CTkEntry(row_inputs, textvariable=self.expert_time_var, width=70, placeholder_text="Ex: 22:30").pack(side="left", padx=(0, 10))

        ctk.CTkLabel(row_inputs, text="Tag:").pack(side="left", padx=(0, 4))
        ...
        lbl_yt_title.pack(anchor="w", padx=12, pady=(6, 2))
        ...
        # --- NOVA COLUNA DIREITA: HISTÓRICO & MURAL ---
        self.right_sidebar = ctk.CTkFrame(main, width=360, corner_radius=14)
        self.right_sidebar.pack(side="right", fill="y", padx=(8, 12), pady=12)
        self.right_sidebar.pack_propagate(False)

        # Mural de Próximos Confrontos (CBF / Streaming Scheduler) - NO TOPO DIREITO
        self.cbf_mural_frame = ctk.CTkFrame(self.right_sidebar, corner_radius=12, border_width=1, border_color="#1f538d")
        self.cbf_mural_frame.pack(fill="x", padx=10, pady=(10, 5))
        
        mural_hdr = ctk.CTkFrame(self.cbf_mural_frame, fg_color="transparent")
        mural_hdr.pack(fill="x", padx=8, pady=(6, 2))
        
        ctk.CTkLabel(mural_hdr, text="⚽ PRÓXIMOS JOGOS (CBF)", font=ctk.CTkFont(size=11, weight="bold"), text_color="#00FF00").pack(side="left")
        ctk.CTkButton(
            mural_hdr, text="🔄", width=22, height=22, font=ctk.CTkFont(size=10, weight="bold"),
            fg_color="#1f538d", hover_color="#153b66", command=self._force_refresh_cbf_mural
        ).pack(side="right", padx=(4, 0))
        
        self.cbf_mural_scroll = ctk.CTkScrollableFrame(self.cbf_mural_frame, fg_color="transparent", height=140)
        self.cbf_mural_scroll.pack(fill="x", padx=4, pady=(2, 6))

        # Seção de Agendamentos e Fila Live (Abaixo do Mural)
        self.schedule_frame = ctk.CTkFrame(self.right_sidebar, corner_radius=12, border_width=1, border_color="#333333")
        self.schedule_frame.pack(fill="x", padx=10, pady=(5, 5))
        
        schedule_hdr = ctk.CTkFrame(self.schedule_frame, fg_color="transparent")
        schedule_hdr.pack(fill="x", padx=8, pady=(8, 2))
        
        ctk.CTkLabel(schedule_hdr, text="⏰ FILA DE AGENDAMENTOS", font=ctk.CTkFont(size=11, weight="bold"), text_color="#00CED1").pack(side="left")
        ctk.CTkButton(
            schedule_hdr, text="➕", width=22, height=22, font=ctk.CTkFont(size=10, weight="bold"),
            fg_color="#333333", hover_color="#444444", command=self._open_schedule_games_modal
        ).pack(side="right")
        
        self.schedule_scroll = ctk.CTkScrollableFrame(self.schedule_frame, fg_color="transparent", height=120)
        self.schedule_scroll.pack(fill="x", padx=4, pady=(2, 4))
        ...
        # Seção de Histórico Expert (Colocada na Direita)
        ...
        ctk.CTkLabel(self.expert_hist_frame, text="AUDITORIAS RECENTES", font=ctk.CTkFont(size=12, weight="bold"), text_color="#aaaaaa").pack(pady=(12, 6))
        self.expert_hist_scroll = ctk.CTkScrollableFrame(self.expert_hist_frame, fg_color="transparent")
        self.expert_hist_scroll.pack(fill="both", expand=True, padx=6, pady=6)
        
        self.btn_refresh_hist = ctk.CTkButton(self.expert_hist_frame, text="Atualizar Lista", font=ctk.CTkFont(size=10), height=24, command=self._load_expert_history_list)
        self.btn_refresh_hist.pack(pady=10)
        ...
        self.tab_reports = self.tabs.add("Relatórios PDF")
    ...
    def _on_mode_change(self, mode: str) -> None:
        ...
        # 1. Painel manual do Expert (FICA NO TOPO APENAS EM MODO EXPERT; SOME NOS OUTROS)
        if hasattr(self, "expert_manual_frame") and self.expert_manual_frame:
            if is_expert:
                self.expert_manual_frame.pack(fill="x", side="top", padx=12, pady=(4, 6), before=getattr(self, "main", None))
            else:
                self.expert_manual_frame.pack_forget()

        # 2. Barra de Pesquisa do YouTube / Canais (self.top) - SEMPRE VISÍVEL EMBAIXO EM TODOS OS MODOS
        ...
        # Regras por modo:
        # Expert    -> Monitoramento, Logs, Erros, Config (oculta: Debug Visual, Fragmentos, Ads/Merchan, Dashboard, Logs IA)
        # Visual    -> Monitoramento, Debug Visual, Fragmentos, Logs, Erros, Config, Logs IA, Ads/Merchan, Dashboard
        # Ads       -> Ads/Merchan, Dashboard, Logs, Erros, Config (oculta: Monitoramento, Debug Visual, Fragmentos, Logs IA)
        # Config IA -> Logs IA, Config, Erros, Logs (oculta: Monitoramento, Debug Visual, Fragmentos, Ads/Merchan, Dashboard)
            ...
            if hasattr(seg_btn, "_buttons_dict"):
                ...
                for tab_name, btn_w in b_dict.items():
                    ...
                    if tab_name == "Monitoramento":
                        ...
                        show = True  # Logs sempre visível em todos os modos
                    elif tab_name == "Erros":
                        show = True  # Erros sempre visível em todos os modos
                    elif tab_name == "Config":
                        show = True  # Config sempre visível em todos os modos
    ...
    def _build_tab_reports(self) -> None:
        """Aba de Visualização Integrada de Relatórios PDF."""
        # Frame principal dividido
        main_split = ctk.CTkFrame(self.tab_reports, fg_color="transparent")
        main_split.pack(fill="both", expand=True, padx=10, pady=10)
        
        # Coluna Esquerda: Lista de Relatórios (Largura fixa)
        left_col = ctk.CTkFrame(main_split, width=280, corner_radius=12, border_width=1, border_color="#333333")
        left_col.pack(side="left", fill="both", padx=(0, 10))
        left_col.pack_propagate(False)
        
        ctk.CTkLabel(left_col, text="📂 RELATÓRIOS SALVOS", font=ctk.CTkFont(size=12, weight="bold"), text_color="#00CED1").pack(pady=(12, 6))
        
        # Botão para atualizar a lista
        btn_refresh_pdf = ctk.CTkButton(left_col, text="🔄 Atualizar Lista", font=ctk.CTkFont(size=10), height=22, command=self._load_pdf_report_list)
        btn_refresh_pdf.pack(pady=4, padx=12, fill="x")
        
        # Scroll para lista de arquivos
        self.reports_pdf_scroll = ctk.CTkScrollableFrame(left_col, fg_color="transparent")
        self.reports_pdf_scroll.pack(fill="both", expand=True, padx=4, pady=4)
        
        # Coluna Direita: Conteúdo e Busca
        right_col = ctk.CTkFrame(main_split, corner_radius=12, border_width=1, border_color="#333333")
        right_col.pack(side="left", fill="both", expand=True)
        
        # Barra de ferramentas superior
        tools_frame = ctk.CTkFrame(right_col, fg_color="transparent", height=40)
        tools_frame.pack(fill="x", padx=12, pady=(10, 5))
        
        ctk.CTkLabel(tools_frame, text="🔍 Buscar no PDF:").pack(side="left", padx=(0, 6))
        
        self.search_pdf_entry = ctk.CTkEntry(tools_frame, placeholder_text="Termo de pesquisa...", width=200)
        self.search_pdf_entry.pack(side="left", padx=4)
        self.search_pdf_entry.bind("<Return>", lambda e: self._search_in_pdf_text())
        
        btn_search = ctk.CTkButton(tools_frame, text="Buscar", font=ctk.CTkFont(size=11, weight="bold"), width=80, command=self._search_in_pdf_text)
        btn_search.pack(side="left", padx=4)
        
        self.btn_open_pdf_external = ctk.CTkButton(
            tools_frame, text="📄 Abrir PDF Externo", 
            font=ctk.CTkFont(size=11, weight="bold"), 
            fg_color="#006400", hover_color="#004d00",
            width=150, command=self._open_current_pdf_externally
        )
        self.btn_open_pdf_external.pack(side="right", padx=4)
        
        # Textbox para visualização do conteúdo
        self.reports_textbox = ctk.CTkTextbox(right_col, font=ctk.CTkFont(family="Consolas", size=11), wrap="word")
        self.reports_textbox.pack(fill="both", expand=True, padx=12, pady=(5, 12))
        
        # Atributos de estado da aba
        self._current_selected_pdf_path = None
        
        # Carga inicial da lista
        self.after(2500, self._load_pdf_report_list)

    def _load_pdf_report_list(self) -> None:
        """Busca arquivos .pdf na pasta de relatórios e exibe na barra lateral da aba."""
        if not hasattr(self, "reports_pdf_scroll"):
            return
            
        for w in self.reports_pdf_scroll.winfo_children():
            w.destroy()
        ...
        if not pdf_files:
            ctk.CTkLabel(self.reports_pdf_scroll, text="Nenhum PDF encontrado.", font=ctk.CTkFont(size=10), text_color="#555555").pack(pady=10)
            return
        ...
        for path in pdf_files:
            ...
            display_name = fname
            if fname.startswith("expert_report_"):
                display_name = fname.replace("expert_report_", "").replace(".pdf", "").replace("_", " ")
            
            def make_select_cb(p_path=path):
                return lambda: self._select_and_view_pdf(p_path)
                
            btn = ctk.CTkButton(
                self.reports_pdf_scroll,
                text=f"📄 {display_name[:35]}",
                font=ctk.CTkFont(size=10),
                fg_color="transparent",
                hover_color="#333333",
                anchor="w",
                height=26,
                command=make_select_cb(path)
            )
            btn.pack(fill="x", padx=4, pady=1)

    def _select_and_view_pdf(self, pdf_path: str) -> None:
        """Extrai o texto do PDF e o exibe no visualizador de texto integrado."""
        self._current_selected_pdf_path = pdf_path
        ...
        try:
            ...
            text_lines.append(f"ARQUIVO: {os.path.basename(pdf_path)}")
            text_lines.append(f"CAMINHO: {pdf_path}")
            text_lines.append("=" * 60 + "\n")
            ...
            full_text = "\n".join(text_lines)
            self.reports_textbox.insert("1.0", full_text)
            
            # Alternar para a aba de relatórios
            self.tabs.set("Relatórios PDF")
            self._log(f"📄 [VISUALIZADOR] PDF carregado na interface com sucesso: {os.path.basename(pdf_path)}")
        except Exception as e:
            self._log(f"[UI ERROR] Falha ao extrair texto do PDF {pdf_path}: {e}")
            self.reports_textbox.insert("1.0", f"Erro ao extrair texto do PDF:\n{e}")

    def _open_current_pdf_externally(self) -> None:
        """Abre o PDF atualmente selecionado no leitor padrão do sistema."""
        path = getattr(self, "_current_selected_pdf_path", None)
        if path and os.path.exists(path):
            try:
                os.startfile(path)
            except Exception as e:
                self._log(f"[UI ERROR] Falha ao abrir PDF externamente: {e}")
        else:
            messagebox.showwarning("Aviso", "Selecione primeiro um relatório PDF da lista lateral.")

    def _search_in_pdf_text(self) -> None:
        """Busca um termo no textbox e rola/destaca a ocorrência."""
        query = self.search_pdf_entry.get().strip()
        if not query:
            return
            
        self.reports_textbox.tag_remove("search", "1.0", "end")
        
        start_pos = "1.0"
        count = 0
        while True:
            pos = self.reports_textbox.search(query, start_pos, stopindex="end", nocase=True)
            if not pos:
                break
            end_pos = f"{pos} + {len(query)} chars"
            self.reports_textbox.tag_add("search", pos, end_pos)
            if count == 0:
                self.reports_textbox.see(pos)
            start_pos = end_pos
            count += 1
            
        self.reports_textbox.tag_config("search", background="#ffd700", foreground="black")
        if count > 0:
            self._log(f"🔎 [VISUALIZADOR] Termo '{query}' encontrado {count} vezes no documento.")
        else:
            messagebox.showinfo("Busca", f"O termo '{query}' não foi encontrado no relatório.")
    ...
    def _add_event_card(self, i: int, ev: Dict[str, Any]) -> None:
        ...
        l1 = ctk.CTkLabel(info_frame, text=badge, font=ctk.CTkFont(size=10, weight="bold"), text_color=("#00FF7F" if status == "live" else ("#FFB300" if status == "upcoming" else "#888888")))
        l1.pack(anchor="w")
        l2 = ctk.CTkLabel(info_frame, text=subtitle, font=ctk.CTkFont(size=12, weight="bold"), anchor="w", wraplength=180, justify="left")
        l2.pack(fill="x")
        l3 = ctk.CTkLabel(info_frame, text=f"{comp} • {sched}", font=ctk.CTkFont(size=10), text_color="#aaaaaa", anchor="w", wraplength=180, justify="left")
        l3.pack(fill="x")
...
📄 SÚMULA ELETRÔNICA CBF: 100% Validada"""
    ...
    def _update_schedule_panel_ui(self) -> None:
        ...
        if current_game_ids != cached_game_ids:
            ...
            if not games:
                ctk.CTkLabel(self.schedule_scroll, text="Nenhum jogo agendado na fila.", font=ctk.CTkFont(size=10), text_color="#555555").pack(pady=10)
                return
        ...
        for g in games:
            ...
            if g_status == "running":
                status_text = "⚙️ PROCESSANDO I.A..."
                status_color = "#FFA500"
                countdown_text = "Consultando dados e gerando PDF..."
            elif g_status == "queued":
                status_text = "⏳ AGUARDANDO VAGA..."
                status_color = "#D2691E"
                countdown_text = "Aguardando outra auditoria concluir..."
            ...
            if g_id not in self._schedule_cards_cache:
                ...
                lbl_team = ctk.CTkLabel(card, text=f"{t1} x {t2}", font=ctk.CTkFont(size=11, weight="bold"), text_color="#FFFFFF")
                lbl_team.pack(anchor="w", padx=8, pady=(4, 1))
                
                lbl_st = ctk.CTkLabel(card, text=f"{status_text} • {d_str} {time_str}", font=ctk.CTkFont(size=9, weight="bold"), text_color=status_color)
                lbl_st.pack(anchor="w", padx=8)
                
                lbl_countdown = ctk.CTkLabel(card, text=countdown_text if countdown_text else "", font=ctk.CTkFont(size=9), text_color="#AAAAAA")
                ...
                if g_status == "completed":
                    ...
                    btn_open = ctk.CTkButton(
                        card, text="📄 Abrir Relatório", font=ctk.CTkFont(size=9, weight="bold"),
                        fg_color="#006400", hover_color="#004d00", height=18, width=100,
                ...
                if g_status == "completed" and cache.get("btn_open") is None:
                    ...
                    def make_open_cmd(p_path=pdf_path):
                        return lambda: self._open_pdf_or_folder(p_path)
                    btn_open = ctk.CTkButton(
                        cache["card"], text="📄 Abrir Relatório", font=ctk.CTkFont(size=9, weight="bold"),
                        fg_color="#006400", hover_color="#004d00", height=18, width=100,
                        command=make_open_cmd()
                    )
                    btn_open.pack(anchor="w", padx=8, pady=(2, 6))
    ...
    def _render_cbf_mural_ui(self) -> None:
        ...
        if not cbf_events:
            ctk.CTkLabel(self.cbf_mural_scroll, text="Nenhum evento encontrado.", font=ctk.CTkFont(size=9), text_color="#777777").pack(pady=10)
            return
            
        for evt in cbf_events[:6]:
            t1 = evt.get("team1", "")
            t2 = evt.get("team2", "")
            comp = evt.get("comp", "")
            d_str = evt.get("date", "")
            ...
            card = ctk.CTkFrame(self.cbf_mural_scroll, fg_color="#181818", corner_radius=6, border_width=1, border_color="#2b5b84")
            card.pack(fill="x", padx=2, pady=3)
            
            hdr_card = ctk.CTkFrame(card, fg_color="transparent")
            hdr_card.pack(fill="x", padx=6, pady=(4, 1))
            
            title_lbl = ctk.CTkLabel(hdr_card, text=f"⚡ {t1} x {t2}", font=ctk.CTkFont(size=10, weight="bold"), text_color="#00FF00")
            title_lbl.pack(side="left")
            
            tag_lbl = ctk.CTkLabel(hdr_card, text=tag, font=ctk.CTkFont(size=8, weight="bold"), text_color="#00CED1")
            tag_lbl.pack(side="right")
            
            info_lbl = ctk.CTkLabel(card, text=f"🏆 {comp[:18]} • 📅 {d_str} {t_str}\n📺 {plat}", font=ctk.CTkFont(size=9), text_color="#CCCCCC", justify="left")
            ...
            btn_apply.pack(anchor="e", padx=6, pady=(0, 4))
