...
class MonitorApp(MonitorCoreMixin, ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        lbl_author = ctk.CTkLabel(
            row_yt_header,
            text="Criado por: Cleber Gonçalves",
            font=ctk.CTkFont(size=11, weight="bold"),
            text_color="#00FF7F"
        )
        lbl_author.pack(side="right", padx=(0, 6))
    ...
    def _on_mode_change(self, mode: str) -> None:
        ...
        # 1. Painel manual do Expert (FIXO NO RODAPÉ, ACIMA DO YOUTUBE APENAS EM MODO EXPERT; SOME NOS OUTROS)
        if hasattr(self, "expert_manual_frame") and self.expert_manual_frame:
            if is_expert:
                self.expert_manual_frame.pack(fill="x", side="bottom", padx=12, pady=(4, 4), before=getattr(self, "top", None))
            else:
