...
class MonitorApp(MonitorCoreMixin, ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        self.mode_switch = ctk.CTkSegmentedButton(
            mode_bar,
            values=["Expert (API-Only)", "Visual (OBS)", "Ads & Merchan"],
    ...
    def _on_mode_change(self, mode: str) -> None:
        ...
        if hasattr(self, "tabs") and hasattr(self.tabs, "_segmented_button"):
            ...
            if hasattr(seg_btn, "_buttons_dict"):
                ...
                for tab_name, btn_w in b_dict.items():
                    if tab_name in ["Debug Visual", "Fragmentos"]:
                        if is_visual:
                            btn_w.grid()
                        else:
                            btn_w.grid_remove()
                    elif tab_name == "Ads/Merchan":
                        if is_ads or is_visual:
                            btn_w.grid()
                        else:
                            btn_w.grid_remove()
                        ...
                        if is_ads or is_visual:
                            ...
                            btn_w.grid_remove()
                    elif tab_name in ["Monitoramento", "Logs IA"]:
                        if is_ads:
                            btn_w.grid_remove()
                        else:
                            btn_w.grid()

        # Se mudou para Ads & Merchan, seleciona a aba Ads/Merchan automaticamente!
        if is_ads and hasattr(self, "tabs"):
            ...
            try:
                if self.tabs.get() in ["Ads/Merchan", "Dashboard"]:
