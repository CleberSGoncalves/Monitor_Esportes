...
📄 SÚMULA ELETRÔNICA CBF: 100% Validada"""
    ...
    def _open_pdf_or_folder(self, path: Optional[str]) -> None:
        """Tenta abrir o PDF do relatório gerado. Se não existir, abre a pasta de relatórios (reports/)."""
        try:
    ...
    def _update_schedule_panel_ui(self) -> None:
        ...
        for g in games:
            ...
            if g_status == "completed":
                ...
                # Criar callback local isolado para evitar closure scope issues no Tkinter loop
    ...
    def _trigger_scheduled_audit(self, game: dict) -> None:
        ...
        if game.get("status") in ["running", "completed", "failed"]:
            return
            
        game["status"] = "running"
        _save_scheduled_games(getattr(self, "_scheduled_games", []))
        self.after(0, self._update_schedule_panel_ui)
        self._log(f"⏰ [AGENDAMENTO] Iniciando auditoria automática de {game.get('team1')} x {game.get('team2')}...")
