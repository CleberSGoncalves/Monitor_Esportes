...
def _load_saved_emails() -> list:
    ...
    default_emails = ["arthur@mediadna.com.br", "cleber.goncalves@ibope.com"]
...
class MonitorApp(MonitorCoreMixin, ctk.CTk):
    ...
    def _update_history_grid(self, force: bool = False) -> None:
        ...
        # Se houver entradas de IA (Expert/Batch), usamos ordem cronológica ascedente (0 -> 120)
        # Se for Live, usamos o padrão (mais recente no topo)
        has_ia = any(e.get("type") == "ia_analysis" for e in tl)
        
        if has_ia:
            def get_min(e):
                # Prioridade: 'minute' no root do entry
                m = e.get("minute")
                if m is not None:
                    try: return int(m)
                    except: pass
                # Depois: 'minute' dentro de root['details']
                d = e.get("details", {})
                ...
                # Por fim: tentar extrair do clock HH:MM (ex: 20:30 -> 1230 minutos do dia)
                c = e.get("clock", "")
                if ":" in str(c):
                    try: 
                        h, m3 = str(c).split(":")
                        return int(h) * 60 + int(m3)
                    except: pass
                return 0
            # IMPORTANTE: No Grid, os primeiros do loop ficam no TOPO.
            # Para cronologia [0 -> 45 -> 90], queremos a ordem ascendente.
            rendering_list = sorted(tl, key=get_min)
        else:
            rendering_list = reversed(tl)
    ...
    def _build_ui(self) -> None:
        ...
        self.mode_switch = ctk.CTkSegmentedButton(
            mode_bar,
            values=["Expert (API-Only)", "Visual (OBS)"],
        ...
        self.expert_manual_frame = None

        self.top = ctk.CTkFrame(self, corner_radius=14)
        self.top.pack(fill="x", padx=12, pady=(8, 8))
        ...
        row2.pack(fill="x", padx=12, pady=(4, 6))
        ...
        self.bar2 = ctk.CTkFrame(self, corner_radius=14)
        self.bar2.pack(fill="x", padx=12, pady=(0, 8))

        ctk.CTkLabel(self.bar2, text="URL manual do evento (opcional):").pack(side="left", padx=(12, 8), pady=10)
        # self.manual_url_var = ctk.StringVar(value="") # Moved to __init__
        self.manual_url_entry = ctk.CTkEntry(self.bar2, textvariable=self.manual_url_var, width=640)
        ...
        # Painel manual do modo Expert (API-Only) para busca sem vídeo
        ...
        ctk.CTkLabel(row_presets, text="Seleção Rápida (Últimos Pesquisados):", font=ctk.CTkFont(size=10, weight="bold"), text_color="#00CED1").pack(side="left", padx=(0, 6))
    ...
    def _on_mode_change(self, mode: str) -> None:
        """Alterna a visibilidade dos campos baseada no modo."""
        ...
        # 1. Painel manual do Expert (Fica no TOPO em modo Expert)
        if hasattr(self, "expert_manual_frame") and self.expert_manual_frame:
            if is_expert:
                self.expert_manual_frame.pack(fill="x", padx=12, pady=(4, 8))
            else:
                self.expert_manual_frame.pack_forget()

        # 2. Barra de Configurações OBS (self.top: Tema, Canal, Competição) - Escondida em modo Expert
        ...
        # 4. URL Manual bar2
        if hasattr(self, "bar2") and self.bar2:
            if is_expert:
                self.bar2.pack_forget()
            else:
                self.bar2.pack(fill="x", padx=12, pady=(0, 8))

        # 5. Switches OBS (Auto-prepare, Auto-start, Modo debug visual)
        for sw in ["chk_auto_prepare", "chk_auto_start", "chk_debug_mode"]:
            if hasattr(self, sw) and getattr(self, sw):
                if is_expert:
                    getattr(self, sw).pack_forget()
                else:
                    getattr(self, sw).pack(side="right", padx=(10, 0), pady=6)
    ...
    def _finalize_expert_batch(self, results: List[Dict[str, Any]]) -> None:
        ...
        try:
            ...
            # Abrir o PDF automaticamente se o usuário desejar
...
📄 SÚMULA ELETRÔNICA CBF: 100% Validada"""
    ...
    def _open_schedule_games_modal(self) -> None:
        ...
        for em in saved_emails:
            ...
            email_vars[em] = var
            ctk.CTkCheckBox(chk_row, text=em, variable=var, font=ctk.CTkFont(size=11)).pack(side="left", padx=8)
        ...
        def _auto_fill_cbf_events():
            ...
            for idx in range(min(5, len(cbf_events))):
                ...
                game_rows[idx][3].delete(0, "end")
                game_rows[idx][3].insert(0, evt.get("date", ""))
            ...
            self._log("📥 Fila de 5 jogos auto-preenchida com os próximos eventos da CBF (Streaming Scheduler)!")
        ...
        if existing:
            ...
            game_rows[0][0].insert(0, "Palmeiras")
            game_rows[0][1].insert(0, "Santos")
            game_rows[0][2].insert(0, "Copa do Brasil")
            game_rows[0][4].insert(0, "21:30")
            game_rows[0][5].delete(0, "end")
            game_rows[0][5].insert(0, "Amazon Prime")
        ...
        for g in games:
            ...
            try:
                ...
                if now < k_dt:
                    ...
                    status_text = "✅ FINALIZADO / PRONTO"
                    status_color = "#ADFF2F"
                    countdown_text = "Relatório disponível no histórico"
            except: pass
    ...
    def _finalize_and_stop(self, reason: str = "manual_stop") -> None:
        ...
        try:
            ...
            if paths and getattr(paths, "pdf_path", None) and self.send_report_email_var.get():
                self._send_report_via_email(paths.pdf_path)
        except Exception as e:
