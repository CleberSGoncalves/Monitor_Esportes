...
class MonitorApp(MonitorCoreMixin, ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        btn_copy_summary = ctk.CTkButton(
            row_actions,
            text="📋 Copiar Resumo",
            command=self._copy_expert_summary_clipboard,
            font=ctk.CTkFont(size=11, weight="bold"),
            fg_color="#2B5B84",
            hover_color="#1E3F5A",
            width=130,
        ...
        # Letreiro Horizontal de Próximos Jogos CBF (Ticker Bar)
        self.cbf_ticker_frame = ctk.CTkFrame(self, height=44, corner_radius=10, fg_color="#141414", border_width=1, border_color="#1f538d")
        self.cbf_ticker_frame.pack(fill="x", padx=12, pady=(2, 4))
        
        lbl_ticker_title = ctk.CTkLabel(
            self.cbf_ticker_frame, 
            text="⚽ PRÓXIMOS JOGOS (CBF):", 
            font=ctk.CTkFont(size=11, weight="bold"), 
            text_color="#00FF7F"
        )
        lbl_ticker_title.pack(side="left", padx=(10, 6), pady=4)
...
📄 SÚMULA ELETRÔNICA CBF: 100% Validada"""
    ...
    def _render_cbf_mural_ui(self) -> None:
        ...
        if not cbf_events:
            ctk.CTkLabel(self.cbf_mural_scroll, text="Nenhum evento da CBF encontrado.", font=ctk.CTkFont(size=11), text_color="#777777").pack(side="left", padx=10)
            return
